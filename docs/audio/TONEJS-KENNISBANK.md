...
6. **Objectieve render-validatie loont, maar kalibreer op échte muziek**: sprongen in `|x[n]−x[n−1]|` + klik-trein-detectie vangen precies de PitchShift-signatuur. Belangrijk (herkalibratie 24-7 na een vals alarm): percussief lesmateriaal telt honderden legitieme sprongen 0.35–0.55, dus een kale klik-teller is géén afkeurcriterium. Wel: (a) een lokale regelmatige trein in het kórrelbereik 0.02–0.11 s (≈ 9–50 Hz — muzikale ritmes zoals 16e noten @120 bpm = 0.125 s vallen erbuiten), (b) ≥10 extreme sprongen ≥ 0.9 (catastrofale modus). Extra vangrail: als de realtime-vangnet-opname hetzelfde profiel flagt als de offline render, stemmen twee onafhankelijke motoren overeen → de vlaggen zijn de muziek zelf → offline render gebruiken, geen waarschuwing. Zie `src/utils/renderValidation.ts` + `validateOrCapture`.

### Aanvullingen 24-7 (middag): limiter + worklet-limitatie

7. **Master-limiter (masterLimiter.ts)**: lookahead brickwall op de master — plafond −1.0 dBFS, 5 ms lookahead via een monotone-deque venster-maximum, 1 ms attack / 150 ms release (one-pole), harde clamp als laatste vangrail. Der kernel is één pure, zelfstandige factory die via `toString()` in de worklet-blob wordt geëmbed én als pure functie over offline buffers loopt (`applyLimiterToChannels`, met lookahead-compensatie) — daardoor is blokgewijze live-verwerking wiskundig identiek aan de offline bewerking (unit-getest: blok==buffer, transparantie bit-identiek onder het plafond).
8. **Tone's context-wrapper: maar één `addAudioWorkletModule` per context.** Een tweede module registreert schijnbaar "ok", maar `createAudioWorkletNode` voor zijn processors gooit NotSupportedError; meerdere processors in dezelfde module werken wél onbeperkt. Bundel eigen processors dus in één module en registreer die met één promise op het context-object (dubbele `registerProcessor` met dezelfde naam gooit óók NotSupportedError — relevant bij HMR/dubbele service-instanties).
9. **Valideer op échte muziek, niet op testtonen** (herkalibratie validator): drums produceren honderden legitieme sample-sprongen van 0.35–0.55. Afkeuren alleen op korrel-treinen (9–50 Hz, lokaal) en herhaalde extreme sprongen; en als het vangnet hetzelfde profiel opneemt als de offline render, is het bewijs geleverd dat de render klopt (overeenstemmings-check in `validateOrCapture`).
