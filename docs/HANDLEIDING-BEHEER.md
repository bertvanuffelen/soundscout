...
| **Sequencer** | altijd aan (sinds 9-8) | De sequencer staat sinds de samenvoeging naar `main` voor iedereen aan; de dev-vlag `sequencer` is verwijderd. Het losse lab op `/sequencer` blijft dev-only (`import.meta.env.DEV`) — dat is een prototype-pagina, geen leerlingfunctie |
