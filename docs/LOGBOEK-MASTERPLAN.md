...
- **Grootste hindernis — export-effectglitch**: effecten (pitch/reverb/fade) klinken bij afspelen goed maar glitchen in de MP3/video-export. Oorzaak in principe gevonden: de export bouwt de effectketen in een **losse offline-implementatie** (`renderOffline`) los van live (`createOnDemandPlayer`); `Tone.PitchShift` in `Tone.Offline` is de hoofdverdachte. **Besluit Bert: apart diepteonderzoek in een verse chat** — daarom een zeer uitgebreid dossier `docs/audio/ONDERZOEK-EXPORT-EFFECTGLITCH.md` + Notion To Do-taak (P1). Geen fix deze ronde.
...
- **Plan geschreven:** `docs/audio/PLAN-AUDIO-ENGINE-V2.md` — gedeelde graph-builder, pitch-prebake, deterministische reverb-IR's, export-validator, 6 fasen. Implementatie nog niet gestart.
