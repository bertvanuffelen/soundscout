...
SoundScout is een React web app (React 19, TypeScript strict, Vite 7, Zustand 5, Tone.js 15, Tailwind 4, Supabase) voor het leren componeren van muziek door kinderen. Leerlingen verzamelen geluidssamples op locaties, plaatsen die op een beat-timeline in de studio, en voeren ze uit op het podium. Docenten maken klassen aan, delen opdrachten (templates of praatplaten) via een klascode, en bekijken inzendingen via een dashboard.
Werkafspraken:

Lees CLAUDE.md in de root van de map voordat je codewijzigingen doet. Die bevat de actuele architectuur, state-management-patronen, Tone.js valkuilen en conventies.
Reageer standaard in het Nederlands.
Werk in kleine stappen. Raak bestaande werkende functionaliteit niet aan tenzij expliciet gevraagd.
Gebruik bestaande UI-componenten (Button, Modal, Card) en de cn() utility in plaats van nieuwe varianten te maken.
Hou je aan i18n — hardcoded Nederlandse of Engelse strings horen in src/i18n/locales/{nl,en}.json, niet in componenten.
Beat-based positionering overal; conversies via src/utils/audio.ts.
Draai na codewijzigingen npx tsc -b --noEmit en npm run test:run om groen te blijven.
Voor database/Supabase-wijzigingen: nieuwe SQL-migratie in supabase/migrations/ met volgend nummer. Rate limiting en RLS zijn verplicht voor publieke RPC's.
Bij features met meerdere stappen: maak eerst een TodoList, vraag bij onduidelijke scope clarificatie, en verifieer na afloop door de code opnieuw tegen de eisen te lezen.
Wees kritisch en eerlijk. Wijs op risico's, edge cases, en betere alternatieven in plaats van klakkeloos uit te voeren.

Uitgangspunten voor design:

Tailwind design tokens uit src/index.css (60-30-10 kleurregel, semantische kleuren).
Touch-first met 44px minimum targets; sm: breakpoint voor desktop.
Geen emoji's in UI of code tenzij expliciet gevraagd.
